...
\medskip
Test Das Eigentliche sind die Ersetzungsregeln! Die Bäume sind nur die Visualisierung.\\
%
\pause%
%
...
\subsection{Grundlegendes zur HPSG}
...
~\\[-20mm]
\begin{itemize}
\item \blau<6>{Phonologie}
\item \blau<7>{Syntax}
\item \blau<8>{Semantik}
\end{itemize}
\end{minipage}%
\parbox[t]{3cm}{
\resizebox{5cm}{!}{
\(
\ms[word]{
{phon}   & \blau<6>{\phonliste{ Grammatik }} \\[1mm]
synsem$|$loc & \ms[loc]{ {cat}  & \blau<7>{\ms[cat]{ head & \ms[noun]{ case & \ibox{1}\\
                                                       }\\[6mm]
                                       subcat & \liste{ DET[{\sc case}~\ibox{1}] } \\
                                     }} \\[6mm]
              {cont} & \blau<8>{\ldots \ms[grammatik]{ inst & X \\
                                   }}\\
            }\\
}
\)
